...
public class SolverApplication implements Callable<Integer> {
    ...
    @Option(
            ...
            description = "Width of Puzzle",
            required = true)
    private Integer width;

    @Option(
            names = {"-ph", "--puzzle-height"},
            description = "Height of Puzzle",
            required = true)
    ...
    @Option(
            names = {"-bs", "--blocks"},
            description =
                    "The blocks in the puzzle, should contains '0'(empty) ,'1'(point), ','(row"
                            + " separator) and ';'(block separator) only, e.g. the `T` shape is"
                            + " \"111,01\"",
            split = ";",
            required = true)
